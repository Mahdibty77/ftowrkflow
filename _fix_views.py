...
def person_shift(request, pk):
    ...
    for c in year_cards:
        ...
        if c["jalali_year"] == view_year and view_year != jy:
            c["is_current"] = False  # selected past year, not "current calendar"
            c["is_selected"] = True
...
# If person_shift_month already inserted, end still points at shift_presence_ping
p.write_text(text[:start] + new + text[end:], encoding="utf-8")
print("rewrote person_shift + person_shift_month cleanly")
